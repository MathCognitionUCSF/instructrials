...
def interactive_individual_line_plot(df, id_col, groupby_col, FC_vars):
    ...
    comparison_selector = widgets.Dropdown(
        options=sorted(df[groupby_col].unique()) + ['All Children'],
        value='All Children',
    ...
    def update_plot(button):
        ...
        # Use the Output widget's context manager to capture the plot
        with out:
            # Get the individual's data
            individual_data = df[df[id_col] == int(id_input.value)][FC_vars].iloc[0]
            
            # Get the comparison group data
            if comparison_selector.value == 'All Children':
                comparison_data = df[FC_vars]
            else:
                comparison_data = df[df[groupby_col] == comparison_selector.value][FC_vars]
            
            # Plotting
            fig, ax = plt.subplots(figsize=(27, 15))
            
            # Plot individual data with vivid blue and thicker line
            ax.plot(FC_vars, individual_data, marker='o', lw=4, color='blue', label=f"Individual {id_input.value}")
            
            # Plot comparison group mean with grey color
            comparison_mean = comparison_data.mean()
            comparison_color = 'grey'
            ax.plot(FC_vars, comparison_mean, marker='o', lw=3, color=comparison_color, label=f"Mean of {comparison_selector.value}")
            
            # Adding shaded region for SEM or STD in grey
            if sem_toggle.value:
                sem = comparison_data.sem()
                ax.fill_between(FC_vars, 
                                comparison_mean - sem, 
                                comparison_mean + sem, 
            ...
            ax.legend(title="Diagnosis", title_fontsize=32, fontsize=32, loc='upper left', bbox_to_anchor=(1, 1))
            ax.set_title("Individual vs Group Comparison", fontsize=38)
            ax.set_xlabel("Task", fontsize=32)
            ax.set_ylabel("Score", fontsize=32)
            plt.xticks(ticks=range(len(FC_vars)), labels=FC_vars, rotation=90, fontsize=28)
    ...
    display(out)  # Display the Output widget below your other widgets
